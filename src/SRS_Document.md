...
This report is organised into seven chapters. Following this introduction, Chapter 2 reviews the literature on document understanding, semantic search, and AI-assisted recruitment, compares the existing systems, and identifies the research gap. Chapter 3 covers the analysis and requirements, from the feasibility study through the functional and non-functional requirements, the tools and technologies adopted, and the use cases. Chapter 4 presents the system design, including the layered architecture, the data-flow diagrams, the UML models, the database design, and the user interface. Chapter 5 describes the implementation, covering the development environment, the backend and frontend, representative code, and the database. Chapter 6 reports the testing strategy, the test cases and their results, and the performance evaluation. Chapter 7 concludes with the outcomes of the project, its limitations, and directions for future work.
...
| # | System | Research Problem | Methodology & Tools | Key Features | Limitations | Relevance to Hireflow |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Smith / Tesseract community [4] | Reading text from scanned images and PDFs | LSTM-based OCR over rasterised document images; Tesseract and pytesseract | Free, offline, multi-format OCR | Accuracy drops on poor scans and handwriting; no semantic understanding | Provides the OCR stage of Hireflow's ingestion pipeline |
| 2 | Greenhouse Software | Managing and filtering high volumes of job applicants | Structured fields with keyword/boolean rules over parsed resumes; commercial ATS and resume parser | Pipeline management, keyword filters, structured candidate records | Keyword matching misses semantics; rigid rules; limited free-form question answering | Establishes the ATS baseline that Hireflow improves on with semantic ranking |
| 3 | Eightfold AI | Bias and inefficiency in candidate-job matching | Learned embeddings of candidates and roles for similarity matching; proprietary deep-learning models | AI candidate matching, skills inference, ranking | Proprietary, closed, and costly, with limited transparency and provenance | Validates ML-based ranking and motivates open, explainable ranking with citations |
| 4 | Johnson, Douze & Jegou / FAISS [6] | Fast similarity search over very large collections of vectors | Approximate nearest-neighbour indexing of dense embeddings; the FAISS library | Efficient, scalable approximate-nearest-neighbour search | A library only, no ingestion, OCR, or application layer | Supports semantic retrieval; Hireflow uses ChromaDB in the same role |
| 5 | Google Cloud Document AI, Amazon Textract, Azure Form Recognizer [1][2][3] | Extracting structured data from unstructured documents | Deep OCR with layout analysis and form/entity extraction, delivered as managed cloud APIs | High-accuracy OCR, entity and table extraction, scalability | Proprietary, per-use cost, cloud lock-in, data leaving the premises | Motivates an on-premise, open OCR pipeline (PyMuPDF + Tesseract) |
| 6 | Hugging Face / Transformer community [7][8] | Understanding document text and semantics | Pretrained transformer encoders for embeddings and LLMs for generation; Transformers and sentence-transformers | Semantic embeddings, classification, generation | Compute-heavy, and general models need domain adaptation | The basis for Hireflow's embeddings and LLM answer generation |
| 7 | Lewis et al. / LangChain [9] | LLMs hallucinate and lack access to private or current data | Retrieve relevant chunks from a vector store, then condition an LLM on them; LangChain, vector databases, LLMs | Grounded, citable answers over private corpora | Answer quality depends heavily on retrieval and chunking, and needs tuning | The core paradigm adopted by Hireflow for document question answering |
...
Table 6.1 presents a representative slice of the full suite, spanning authentication, document processing, hybrid search, RAG question-answering, and candidate matching. Each case names the use case it exercises and gives an example input, the expected result, and the actual observed result; all reflect the specified and verified behaviour of the implemented system.

Table 6.1: Test Cases and Results

| Use Case | Test Case | Input | Expected Result | Actual Result | Status |
| --- | --- | --- | --- | --- | --- |
| UC-01 | TC-AUTH-006: Login with valid credentials issues tokens | POST /api/auth/login with email hr@test.hireflow.io and password hr-test-password | HTTP 200; access + refresh token in body; token_type=bearer | Both tokens returned; token_type=bearer | Pass |
| UC-01 | TC-AUTH-008: Unknown email returns invalid_credentials (no user enumeration) | POST /api/auth/login with email nobody@test.hireflow.io and any password | HTTP 401 with identical shape/message to the wrong-password case | 401 with identical envelope; response timing uniform | Pass |
| UC-01 | TC-AUTH-014: Refresh rotates token; old refresh becomes unusable | POST /api/auth/refresh with a valid refresh token, then the same request again reusing that same token | Step 1 returns HTTP 200 with a new pair; step 2 returns HTTP 401 invalid_token | Old token revoked in Redis; reuse rejected with 401 | Pass |
| UC-03 | TC-DOCS-001: Uploaded PDF appears in list as ready | POST /api/documents with alice_resume.pdf (two-page PDF, 180 KB), then poll the document list | The row appears with status=ready within 60 seconds | Pipeline settled to ready; detail view opens | Pass |
| UC-03 | TC-DOCS-004: Upload over the size limit returns 413 | POST /api/documents with an 11 MB scanned PDF, against the 10 MB limit | HTTP 413 with error code file_too_large | 413 file_too_large returned | Pass |
| UC-03 | TC-DOCS-005: Unsupported MIME type returns 415 | POST /api/documents with candidates.csv, media type text/csv | HTTP 415, unsupported file type | 415 returned; allowlist enforced | Pass |
| UC-09 | TC-DOCS-046: Candidate auto-created from a resume | POST /api/documents with alice_resume.pdf, wait for status ready, then GET /api/candidates | A candidate row linked via source_document_id | Candidate auto-created by the on-ready hook | Pass |
| UC-04 | TC-SEARCH-005: An HR user sees only their own documents | Signed in as hr@test.hireflow.io, search for admin_python, a term that appears only in the administrator account documents | No results | Owner scope enforced; empty result set | Pass |
| UC-04 | TC-SEARCH-015: Trigram fallback fires when full-text search yields nothing | Search for pyhton, a transposition of python, where no filename contains either spelling | The document returns via the word_similarity fuzzy fallback | Fuzzy path recovered the python document | Pass |
| UC-04 | TC-RAG-001: Grounded answer returned with citations | POST /api/rag/query with the question: Which candidates have Kubernetes experience? | HTTP 200; answer cites at least one source [filename]; confidence non-null | Answer grounded with typed citations | Pass |
| UC-04 | TC-RAG-002: Out-of-scope question returns the refusal sentinel | POST /api/rag/query with the question: Explain quantum physics | HTTP 200; exact sentinel; confidence null; no fabricated citations | Sentinel "Not in the provided documents." returned | Pass |
| UC-12 | TC-JOBS-010: Match scores candidates with a breakdown | Run match for a job requiring Python, Django and PostgreSQL against eight indexed resumes | Each result has a score plus a breakdown (skill, experience, vector) | Scores and persisted match_breakdown returned | Pass |
